...
1. `command -v tabcli`を実行する。見つからない場合は順に`command -v tabcli.exe`、`$HOME/.local/bin/tabcli`を確認する。WSLではさらに、Windowsの`%LOCALAPPDATA%\Programs\tabcli\tabcli.exe`を`powershell.exe`で取得し、`wslpath -u`で変換したpathを確認する。
