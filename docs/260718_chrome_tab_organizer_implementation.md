# 機能実装計画: Chrome Tab Organizer MVP（macOS）

## 機能概要

Google Chromeで現在開いているタブを、ローカルのCLIまたはMCP対応AIエージェントから参照・分類し、ユーザーが差分を確認してからChromeネイティブのタブグループへ安全に反映できるようにする。

Chrome拡張機能がChrome APIとの境界を担当し、Chromeがオンデマンド起動する単一のGo実行ファイル `tabctl` がNative Messaging Host、認証付きStreamable HTTP MCP、stdio MCPプロキシ、CLIを提供する。製品自身はLLMを呼び出さず、タブ情報やページ本文を外部へ自動送信しない。
...
skills/chrome-tab-organizer/# AIエージェント向けSkill
docs/                       # 要求仕様と実装計画
```
...
```mermaid
flowchart LR
    User["ユーザー"] --> CLI["tabctl CLI"]
    Agent["AIエージェント"] <-->|"stdio MCP"| Proxy["tabctl mcp serve"]
    CLI --> Client["共通MCP Client"]
    Proxy --> Client
    Client -->|"Bearer付き Streamable HTTP"| Host["tabctl Native Host / MCP Server"]
...
- [x] `cmd/tabctl` と `internal/app` に単一バイナリのモード選択を実装する。
...
- [x] `tabctl install` のユーザー別Native Messaging登録、`tabctl status`、`tabctl tabs list`、`tabctl groups list` を実装する。
...
3. `CGO_ENABLED=0 go build ./cmd/tabctl` で `tabctl` を生成し、`tabctl install` を実行する。
4. 開発ビルドの拡張機能をChromeへ読み込み、通常・pinned・グループ化済みタブを用意する。
5. `tabctl tabs list --json` と `tabctl groups list --json` を実行する。
6. Chromeを終了し、`tabctl status` がブラウザー未接続を返し、Native Hostとdiscovery fileが残らないことを確認する。
...
- [x] `tabctl mcp serve` をstdio MCP serverとして実装し、共通ツールカタログをそのまま公開する。
...
1. `go test ./...` を実行する。
2. MCP inspectorまたは小さなテストクライアントから `tabctl mcp serve` を起動する。
...
- [x] `tabctl tabs list --inactive-for` と `--sort` をMCP入力へ変換する。
...
5. `tabctl tabs list --inactive-for 7d --json` とMCPの `inactiveForSeconds=604800` の結果が一致することを確認する。
...
- [x] `chrome_tab_content_get` と `tabctl tabs content` を実装し、権限不足時は対象originとUI操作手順を返す。
- [x] `chrome_tab_content_compare` と `tabctl tabs compare` を実装し、可視テキスト全体を拡張機能内でSHA-256化して本文を返さない。
- [x] `chrome_tab_content_diff` と `tabctl tabs diff` を実装し、拡張機能内で生成した上限付き変更行だけを返す。
...
- [x] `chrome_tab_groups_preview` と `tabctl groups preview --plan` を実装する。
...
- [x] `chrome-tab-organizer` Skillを作成し、一覧、放置タブ、本文、preview、承認、apply、Undoの手順を記述する。
...
3. MCP対応AIエージェントへ `tabctl mcp serve` を登録し、要求仕様のUC-01からUC-07を実行する。
...
- Go module path: `github.com/masahide/chrome-tab-organizer`
- Native Messaging Host名: `io.github.masahide.chrome_tab_organizer`
