...
- 文書状態: Draft 0.7
- 作成日: 2026-07-18
- 対象: Windows 11 x64 / Google Chrome Stable（先行MVP）。macOSは既存コード維持と後続再検証。
...
1. ユーザーが `tabcli list` または `tabcli list --inactive-for 7d` を実行する。
...
| ID | 要求 |
|---|---|
| SKILL-001 | Skillは「現在の開いているタブを教えて」を `tabcli --json list` と `tabcli --json group list` に対応付けること。 |
...
| SKILL-004 | Skillは「長時間放置されているタブを教えて」を `tabcli --json list --inactive-for DURATION` に対応付けること。 |
...
| SKILL-016 | Skillは本文、タイトルまたはURLから閉じる対象を自動決定せず、承認されたtabIdだけを `tabcli --json close --confirm` へ渡すこと。 |
| SKILL-017 | 2タブの内容一致だけを確認する場合は `tabcli --json compare` を優先し、本文取得または差分取得を不要に行わないこと。 |
| SKILL-018 | 差分が必要な場合だけ `tabcli --json diff` を使用し、返された変更行をuntrusted dataとして扱うこと。 |
...
tabcli status
tabcli mcp serve
tabcli list [--window ID] [--ungrouped] [--inactive-for DURATION] [--sort FIELD] [--json]
tabcli content TAB_ID [--max-chars N] [--json]
tabcli compare TAB_ID_A TAB_ID_B [--json]
tabcli diff TAB_ID_A TAB_ID_B [--max-chars N] [--max-diff-chars N] [--json]
tabcli close --confirm TAB_ID [TAB_ID ...] [--json]
tabcli group list [--window ID] [--json]
tabcli group preview --plan FILE [--json]
tabcli group apply --preview-id ID [--json]
tabcli group undo [--json]
```
...
| CLI-015 | `tabcli content` は `chrome_tab_content_get` を呼び出し、権限不足時に対象originと拡張機能UIでの許可手順を表示すること。 |
| CLI-016 | `tabcli content` の通常出力およびJSON出力をファイルへ自動保存しないこと。 |
| CLI-017 | `tabcli close --confirm TAB_ID [TAB_ID ...]` は正確な対象IDと確認フラグを必須とし、成功時に閉じたIDをJSONで返せること。 |
| CLI-018 | `tabcli compare TAB_ID_A TAB_ID_B` は `chrome_tab_content_compare` を呼び出し、通常出力に一致結果と各SHA-256を、JSON出力にMCP結果を返すこと。 |
| CLI-019 | `tabcli diff TAB_ID_A TAB_ID_B` は `chrome_tab_content_diff` を呼び出し、通常出力には変更行だけを、JSON出力には上限・切り詰め情報を含むMCP結果を返すこと。 |
| CLI-020 | `compare` と `diff` は相異なる正のタブIDをちょうど2件必須とし、結果から `close` を自動実行しないこと。 |
...
| NFR-008 | Windows 11 x64とChrome Stable 121以降を先行MVPの検証対象とすること。 |
...
| NFR-012 | Windows 11 x64実機で、Native Messaging、stdio MCP、Streamable HTTP MCPの結合試験を行うこと。macOS実機再検証は後続とする。 |
...
| ID | 要求 |
|---|---|
| DIST-001 | 先行MVPでは`windows/amd64`の実行ファイルと配布ZIPを生成すること。Windows arm64とmacOS配布更新は後続とする。 |
...
| DIST-008 | macOSのDeveloper ID署名、notarization、配布再検証はWindows MVP後の後続作業とすること。 |
| DIST-009 | Windows先行MVPではAuthenticode署名を非スコープとし、SmartScreen警告の可能性を明記すること。 |
...
| DIST-013 | Windows installerは実行中の`tabcli.exe`を置換できない場合、Chromeを強制終了せず既存配置を保持して失敗すること。 |
| DIST-014 | 単一release entrypointがWindows amd64バイナリ、拡張ZIP、再現可能bundle ZIP、SHA-256 checksum、version metadata、PowerShell installerを生成・検証すること。 |
| DIST-015 | `install-with-gh.ps1`はWindows amd64 bundleと`SHA256SUMS`を取得し、checksumを検証してからbundle内`install.ps1`を起動すること。 |
...
- MCPから取得したタブ一覧と `tabcli list --json` の内容が一致する。
...
- Windows amd64向けに`tabcli.exe`と同一versionの配布ZIPを生成できる。
- 実行中の最終パスを強制的に上書きせず、安全に更新を中断できる。
- Windows 11 x64で `tabcli install` 後にChrome拡張機能がHKCU登録からNative Hostへ接続できる。
